...
class TestCommand extends Command {
  ...
  nodeParse(data) {
    ...
    let Description = toMarkdown(data.Description).slice(0, 1200);
    ...
    let Link = this.nodeGenerateLink(data.TextRaw);
  ...
  nodeGenerateLink(TextRaw) {
    let Class = TextRaw.split(".")[0].replace(`Class: `, ``).toLowerCase();
    ...
    let Link = `https://nodejs.org/api/${Class}.html#${Class}_${Property}`;
    return Link;
  }
}
