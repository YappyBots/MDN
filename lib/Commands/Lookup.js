...
class TestCommand extends Command {
  ...
  node(msg, args) {
    ...
    .then((data) => {
      return msg.channel.sendMessage(data, { split: false });
    })
    .catch(err => {
  ...
  nodeParse(data) {
    ...
    let Description = toMarkdown(data.Description).slice(0, 300);
    ...
    console.log(`Parser | DescriptionRegEx2`, DescriptionRegEx2);
    return [
      `**\`${data.TextRaw}\`${DocInfo}**`,
      ``,
      // "```js",
      // data.TextRaw,
      // "```",
