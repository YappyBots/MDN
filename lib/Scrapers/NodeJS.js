...
class NodeJSScraper {
  ...
  async downloadDocs() {
    for (let source in this.sources) {
      Log.debug(`NodeJS | Downloading ${this.sources[source]}... ${++source}/${this.sources.length}`);
  ...
  async downloadSource(source) {
    return new Promise((resolve, reject) => {
      if (!source) return reject(`No source provided!`);
  ...
  async addToCache(data) {
    this.docsData.push(data);
    Promise.resolve(this.docsData);
  }

  async searchDocs(query) {
    let result;
    for (let doc in this.docsData) {
      let obj = SearchUtility.GetSimilarObjects(this.docsData[doc], `textRaw`, query)[0];
      if (obj) {
        result = obj;
        result.source = this.docsData[doc].source;
        break;
      }
    }
